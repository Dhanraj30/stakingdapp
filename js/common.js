    ...
    if (_provider == "metamask_wallet") {
        if (window.ethereum && window.ethereum.providers) {
            ...
            if (metamaskProvider){
                ...
                console.log("metamask wallet not foun");

                window.open("http://metamask.io/download/", "_blank").focus();
...
async function commonInjectedConnect( _provider, _provider_name ) {
    ...
    oContractToken = new web3.eth.Contract(
        SELECT_CONTRACT[_NETWORK_ID].TOEKEN.abi,
        SELECT_CONTRACT[_NETWORK_ID].TOEKEN.address
    );
    return true;
 ...
 function setWeb3Events(_provider) {
    _provider.on("accountsChanged", (accounts) => {
        ...
        if (!accounts.length) {
            ...
            cuurentAdddress = accounts[0];
            let sClass = getSelectedTab();
        }
    });

    // subscribe to chain Id change
 ...
 function formatEthErrorMsg(error) {
    try {
        ...
        var eM4 = error.message.indexOf("Internal JSON_RPC error");
 ...
 function getSelectedContract(sClass) {
