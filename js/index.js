...
async function loadInitialData(sClass) {
    ...
    try {
        ...
        //ELEMENTS __CLASS
        document.getElementById(
            "total-locked-tokens-value"
        ).innerHTML = `${totalLockedTokens / 10 ** 18
        ...
        .forEach(function (element) {
         ...
         document.querySelector(".Lock-period-value").forEach(function (element) {
            element.innerHTML = `${dayDisplay}  `;
         });

         let rewardBal = await cObj.methods.getUsersEstimatedRewards()
         ...
         document.getElementById("user-token-balance"
         ...
         document.getElementById(".apy-value").forEach(function (element) {
...
function generateCountDown(ele, endDate) {
    clearInterval(countDownGlobal);
...
async function stackTokenMain(_amount_wei, sClass) {
    ...
    let oContractToken = getContractObj(sClass);
    ...
    .on("receipt", (receipt) => {
        ...
        const receiptObj = {
            ...
            effectiveGasUsed: receipt.effectiveGasPrice,
...
async function unstackTokens() {
    try {
        ...
        if (balMainUser < nTokens) {
            notyf.error(
                `insufficient staked tokens on ${SELECT_CONTRACT_NAME[_NETWORK_ID].network_name}!`);
                return;
...
async function claimTokens() {
    try {
        ...
        let rewardBal = await oContractToken.methods
        .getUsersEstimatedRewards()
        .call({from: currentAddress});
        ...
        if (!rewardBal) {
            notyf.dismiss(notification);
            notyf.error(
                `insufficient reward tokens to claim`
            );
            return;
        }

        claimTokenMain(oContractToken, sClass);
    } catch (error) {
        console.log(error);
        notyf.dismiss(notification);
        notyf.error(formatEthErrorMsg(error));
